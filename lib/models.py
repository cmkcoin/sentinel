...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, cmkd):
        golist = cmkd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_cmkd(cmkd, item)
    ...
    @classmethod
    def import_gobject_from_cmkd(self, cmkd, rec):
        import decimal
        import cmklib
        import inflection
        ...
        # shim/cmkd conversion
        object_hex = cmklib.SHIM_deserialise_from_cmkd(object_hex)
        objects = cmklib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from cmkd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from cmkd, with every run
        ...
        # get/create, then sync payment amounts, etc. from cmkd - Cmkd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(cmkd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from cmkd! %s" % e)
            govobj.vote_delete(cmkd)
            return (govobj, None)
    ...
    def vote_delete(self, cmkd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(cmkd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, cmkd, signal, outcome):
        import cmklib
        ...
        output = cmkd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = cmklib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(cmkd, signal)

    def sync_network_vote(self, cmkd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = cmkd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 cmk addr, non-multisig
            if not cmklib.is_valid_cmk_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Cmk address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (cmklib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/CmkDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import cmklib
        obj_data = cmklib.SHIM_serialise_for_cmkd(self.serialise())
    ...
    def prepare(self, cmkd):
        try:
            object_hash = cmkd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not cmklib.is_valid_cmk_address(addr, config.network):
    ...
    def hash(self):
        import cmklib
        return cmklib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, cmkd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - cmkd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, cmkd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - cmkd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, cmkd):
        now = int(time.time())
        return (self.created_at < (now - cmkd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, cmkd):
        if self.is_expired(cmkd):
            return False

        return True

    def is_deletable(self, cmkd):
        if self.is_expired(cmkd):
            return True
