...
class CmkDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some cmkd methods
    ...
    @classmethod
    def from_cmk_conf(self, cmk_dot_conf):
        from cmk_config import CmkConfig
        config_text = CmkConfig.slurp_config_file(cmk_dot_conf)
        creds = CmkConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = cmklib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = cmklib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = cmklib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_cmkd_proto_version_with_sentinel_ping)
