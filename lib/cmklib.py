...
def is_valid_cmk_address(address, network='mainnet'):
    ...
    cmk_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Cmk address
        return False

    if (address_version != cmk_version):
        return False
...
# shims 'til we can fix the cmkd side
def SHIM_serialise_for_cmkd(sentinel_hex):
    ...
    # superblock => "trigger" in cmkd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # cmkd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    cmkd_hex = serialise(obj)
    return cmkd_hex


# shims 'til we can fix the cmkd side
def SHIM_deserialise_from_cmkd(cmkd_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(cmkd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('cmk.conf')
