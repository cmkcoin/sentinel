...
sentinel_cfg = CmkConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_cmkd_proto_version_with_sentinel_ping = 70207


def get_cmk_conf():
    home = os.environ.get('HOME')

    cmk_conf = os.path.join(home, ".cmkCore/cmk.conf")
    if sys.platform == 'darwin':
        cmk_conf = os.path.join(home, "Library/Application Support/CmkCore/cmk.conf")

    cmk_conf = sentinel_cfg.get('cmk_conf', cmk_conf)
