...
def has_cmk_conf():
    ...
    valid_cmk_conf = False

    # ensure cmk_conf exists & readable
    #
    # if not, print a message stating that Cmk Core must be installed and
    # configured, including JSONRPC access in cmk.conf
    try:
        f = io.open(config.cmk_conf)
        valid_cmk_conf = True
    except IOError as e:
        print(e)

    return valid_cmk_conf
...
def main():
    ...
    if not has_cmk_conf():
        print("CmkCore must be installed and configured, including JSONRPC access in cmk.conf")
        sys.exit(1)
