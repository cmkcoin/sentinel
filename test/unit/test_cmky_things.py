...
def test_valid_cmk_address():
    from cmklib import is_valid_cmk_address

    main = valid_cmk_address()
    test = valid_cmk_address('testnet')

    assert is_valid_cmk_address(main) is True
    assert is_valid_cmk_address(main, 'mainnet') is True
    assert is_valid_cmk_address(main, 'testnet') is False

    assert is_valid_cmk_address(test) is False
    assert is_valid_cmk_address(test, 'mainnet') is False
    assert is_valid_cmk_address(test, 'testnet') is True


def test_invalid_cmk_address():
    from cmklib import is_valid_cmk_address

    main = invalid_cmk_address()
    test = invalid_cmk_address('testnet')

    assert is_valid_cmk_address(main) is False
    assert is_valid_cmk_address(main, 'mainnet') is False
    assert is_valid_cmk_address(main, 'testnet') is False

    assert is_valid_cmk_address(test) is False
    assert is_valid_cmk_address(test, 'mainnet') is False
    assert is_valid_cmk_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from cmklib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % cmklib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(cmklib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(cmklib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(cmklib.blocks_to_seconds(16616)) == 2612035
